...
# Slot is determined at runtime (stack-peek in boot, ZP $43 for ProDOS calls)
# so the same ROM image boots correctly from any slot.
...
class Asm:
    ...
    def AND_imm(self, v): self._emit(0x29, v)
    def ASL_a(self):      self._emit(0x0A)  # ASL accumulator
    def TSX(self):        self._emit(0xBA)
...
# ---- Boot entry ($Cn08) ----
# Apple II PR#n falls through signatures to here. The very first thing we do
# is JSR find_slot so the return address pushed on the stack captures our
# current $Cn. After find_slot returns with X = slot*16, later JSR/JMP to
# hardcoded $C4xx labels is harmless because the driver uses X for register
# addressing, not PC — we'll hit the right card's registers regardless of
# which $Cn page the code is physically executing in.
a.JSR('find_slot')       # returns X = slot*16
a.STX_zp(Unit)
a.LDA_imm(CMD_READ)
a.STA_zp(Command)
a.LDA_imm(0x00)
...
# ---- ProDOS driver entry ----
# ProDOS sets ZP $43 (Unit) = slot*16 | drive_bit before JSR to our entry.
# Mask off the drive bit to get slot*16 for register addressing.
...
# ---- find_slot: returns X = slot*16 (uses JSR-return-address peek) ----
# JSR pushes PC-1 = $Cnxx (last byte of the JSR instruction). TSX+LDA $0102,X
# reads the high byte of that return address — always $Cn of the slot that
# called us. This is why find_slot must be invoked via JSR FROM THE CARD'S
# OWN PAGE, before any cross-page transfer — if we JMP'd to another slot's
# ROM first, the return address would reflect that slot, not ours.
a.label('find_slot')
a.TSX()
a.LDA_abx(0x0102)        # A = high byte of return addr = $Cn
a.AND_imm(0x0F)          # A = n
a.ASL_a()
a.ASL_a()
a.ASL_a()
