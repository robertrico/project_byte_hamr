        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                READ_CAT: begin
                    ...
                    if (cat_byte_idx > 5'd0)
                        cat_buf[cat_byte_idx - 5'd1] <= cat_rd_data;
                    cat_byte_idx <= cat_byte_idx + 5'd1;
                    if (cat_byte_idx == 5'd20) begin
                        cat_buf[19] <= cat_rd_data;
                        state <= PARSE_CAT;
                    end
                end
                ...
                COPY_LO: begin
                    ...
                    if (sd_read_data_valid) begin
                        ...
                        if (skip_bytes > 16'd0) begin
                            ...
                            // Pause SD reads while we wait for hi byte + SDRAM write
                ...
                COPY_HI: begin
                    sd_read_data_ready <= 1'b1;  // resume SD reads
                    if (sd_read_data_valid) begin
                        byte_in_sector <= byte_in_sector + 10'd1;
                        sd_read_data_ready <= 1'b0;  // pause again
                ...
                WRITE_SDRAM: begin
                    if (sdram_req && sdram_req_ready) begin
                        sdram_req        <= 1'b0;
                ...
                NEXT_CLUSTER: begin
                    ...
                    fat_target_offset <= {current_cluster[6:0], 2'b00};
                ...
                FAT_DONE: begin
                    if ((fat_entry & 32'h0FFFFFFF) >= 32'h0FFFFFF8) begin
                        state <= DONE;
                    end else begin
                        current_cluster   <= fat_entry & 32'h0FFFFFFF;
                        sector_in_cluster <= 9'd0;
                        chain_wr_addr     <= chain_idx;
                        chain_wr_data     <= fat_entry & 32'h0FFFFFFF;
                        chain_wr_en       <= 1'b1;
                        chain_idx         <= chain_idx + 10'd1;
                        state             <= READ_SECTOR;
                    end
