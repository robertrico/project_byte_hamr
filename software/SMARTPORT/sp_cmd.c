...
static int do_handshake_send(int pkt_len)
{
    ...
    // Disable interrupts during TX to prevent FIFO underrun from USB/IRQ
...
void sp_main(PIO pio_rx, uint sm_rx, PIO pio_tx, uint sm_tx)
{
    ...
    for (;;) {
        ...
        // Sanity check: first 8 wire bytes must have bit 7 set
        // (C3, DEST|80, SRC|80, PTYPE|80, AUX|80, STAT|80, ODDCNT|80, GRP7CNT|80)
        if (pkt_len >= 8) {
            int bad = 0;
            for (int i = 1; i < 8; i++)
                if (!(pkt_bytes[i] & 0x80)) bad = 1;
            if (bad) {
                ack_deassert();
                pio_rx_restart();
                continue;
        ...
        cmd_struct_t cmd;
        decode_cmd(pkt_bytes, pkt_len, &cmd);
        ...
        log_event(cmd.cmd, cmd.dest, cmd.status_code, flags,
                  (int8_t)chk, sample_count, pkt_len, handled ? g_tx_len : 0,
                  blknum);
    }
}
